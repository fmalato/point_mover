...
class BulletGeometryMover(gym.Env):
    ...
    def __init__(self, max_timesteps, frame_skip=1, camera_distance=5, on_linux=False, limit_fps=False):
        super().__init__()
        self.goal_state = [np.random.uniform(low=-3.0, high=3.0), np.random.uniform(low=0.0, high=3.0)]
    ...
    def reset(self):
        # Reset rod position
        self.rod_pos = [np.random.uniform(low=-3.0, high=3.0), 0, np.random.uniform(low=0.0, high=3.0)]
        #rod_pos = [2, 0, 3]
        p.resetBasePositionAndOrientation(self.boxId[1], self.rod_pos, [0, 0, 0, 1])
        self.goal_state = [self.rod_pos[0], self.rod_pos[2]]
        # Reset pointer position
        self.restart_position = [np.random.uniform(low=-3.0, high=3.0), 1, np.random.uniform(low=0.0, high=3.0)]
